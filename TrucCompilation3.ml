(*Version avec 8 victoires, je dois améliorer le trouve_chemin afin qu'il aille à des checkpoints, et améliorer l'algo minmax pour qu'il les fasse tous monter en même tempsz*)
  ...
  if not ((est_dans_losange (x2, y2, z2) dim)&&(est_dans_losange (x1, y1, z1) dim)) then false 
    ...
    if d mod 2 = 1 then false else 
      ...
      if List.exists (fun (case, _) -> case = pivot) case_coloree_list &&
        ...
        if case_entre config (x1, y1, z1) (xp1, yp1, zp1)  d1 && case_entre config pivot (xp2, yp2, zp2)  d2 then true
        else false
      else
        false;;
        ...
        let rec calcul_saut (config : configuration) (case_init : case) (case_pivot1 : case) (direction : case) (dim : int): (case * coup) list = 
          match dim with 
          | 0 -> [] 
          | _ -> 
              ...
              if List.exists (fun (case_coloree, _) -> case_coloree = case_pivot) case_coloree_list then 
                let (_, d) = vec_et_dist case_init case_pivot in 
                  ...
                  if est_saut case_init destination config then 
                    [(case_init, Du (case_init, destination))]
                  else 
                    []
              else 
        ...
        let calcul_liste_saut (config: configuration) (case_init : case) directions : (case * coup) list = 
          directions
          |> List.fold_left (fun acc direction -> calcul_saut config case_init case_init direction (guess_dim config) @ acc) [];;
...
remplir_triangle_haut 3 (dim+1,-dim,-1);;
(*
let first_three lst = 
  match lst with 
  | a::b::c::q-> [a;b;c];;

let ilEnManque1PourFinirLaPartie config joueur_max= (*on fait en sorte qu'il nous renvoie la case vide à obtenir *)
  let listeTriangleHaut = remplir_triangle_haut dim (dim+1, -dim, -1) in 
  let liste =first_three listeTriangleHaut in
  let rec bonne_couleur liste1 config nbCasesVides joueur_max =
    match liste1 with
    | [] -> if nbCasesVides>1 then false else true
    | x::q -> 
      match quelle_couleur x config with
      | Libre -> bonne_couleur q config (nbCasesVides+1) joueur_max
      | couleur when couleur=joueur_max -> bonne_couleur q config nbCasesVides joueur_max
      | _ -> false in bonne_couleur liste config 0 joueur_max ;; 

*)
(* Prendre les trois premiers éléments d'une liste, retourne None si moins de trois éléments *)
...
remplir_triangle_haut dim (dim+1, -dim, -1);;

drop_first dim (remplir_triangle_haut dim (dim+1, -dim, -1));;

let config_test =
  ([((4, -1, -3), Vert); ((4, -2, -2), Vert); ((-3, 0, 3), Vert);
  ((5, -2, -3), Vert); ((5, -3, -2), Vert); ((6, -3, -3), Vert)],
 [Vert; Marron]);;
...
verif_preliminaire config_test Vert;;
...
let finitionLockin config joueur_max = (*il me renvoie la bonne liste de coup, maintenant il ne reste plus qu'à vérifier prendre le premier coup*)
  ...
  if test then meilleur_chemin config case_initiale case_finale 10 else  failwith "problème";;

finitionLockin config_test Vert;;
...
let ia_next_coup config = 
  ...
  | _ -> 
   ...
   if var then
     ...
     | None -> failwith "sur la fin on n'a pas réussi à trouver le dernier coup"
...
(*on teste le chemin le plus rapide*)
...
let config_test =
  ([((4, -1, -3), Vert); ((4, -2, -2), Vert); ((-3, 0, 3), Vert);
  ((5, -2, -3), Vert); ((5, -3, -2), Vert); ((0, 0, 0), Vert)],
 [Vert; Marron]);;
